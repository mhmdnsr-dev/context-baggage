...
func TestPullPreflightAcceptsOmittedLegacyTaskWorkspaceID(t *testing.T) {
	s, folder := newStore(t)
	if _, err := Init(s, folder); err != nil {
		t.Fatal(err)
	}
	writePortableV2Workspace(t, folder, "w_shared", store.PortableWorkspace{
		...
		"tasks/t_legacy/task.yaml": "id: t_legacy\nname: legacy task\n",
		"active-task":              "t_legacy\n",
	})

	establishLocalBase(t, s)
	if _, err := Pull(s); err != nil {
		t.Fatalf("legacy task without workspaceId should remain importable: %v", err)
	}
	if task, err := s.ReadTask("w_shared", "t_legacy"); err != nil || task.ID != "t_legacy" {
		t.Fatalf("legacy task was not imported: %+v %v", task, err)
	}
}
